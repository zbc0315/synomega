...
per-target agreement, and the wall-clock efficiency contrast) from:
  ...
  - ../efficiency_coverage/full.csv            (SynOmega, unconstrained model)
  - ../efficiency_coverage/simplify.csv        (SynOmega, simplification-constrained)
...
def main():
    ...
    full_solved, full_sec = load_synomega("full.csv")
    simp_solved, simp_sec = load_synomega("simplify.csv")

    # SynOmega scored all 1000 targets; restrict to the 200 in the AiZynth run
    common = [k for k in az_solved if k in simp_solved and k in full_solved]
    n = len(common)
    full_sec_all = [full_sec[k] for k in common if k in full_sec]
    ...
    print(f"  SynOmega (full)      {sum(full_solved[k] for k in common)}/{n} = "
          f"{100 * sum(full_solved[k] for k in common) / n:.1f}%")
    ...
    print(f"  SynOmega (full)      median {st.median(full_sec_all):.2f}   mean {st.mean(full_sec_all):.2f}")
    ...
    print(f"  -> AiZynth median is {st.median(az_time) / st.median(simp_sec_all):.0f}x the simplify "
          f"model and {st.median(az_time) / st.median(full_sec_all):.0f}x the full model\n")
