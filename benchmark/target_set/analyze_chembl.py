#!/usr/bin/env python3
"""Full analysis of the ChEMBL-target re-run: efficiency (simplify vs full),
...
_PATHS = {
    "full_1000.csv": "../efficiency_coverage/full.csv",
...
def efficiency(n_label="1000"):
    full = load("full_1000.csv")                # full model on all 1000
    simp = load("simplify_1000.csv")            # simplify model on all 1000
    fmap = {r["smiles"]: r for r in full}
    smap = {r["smiles"]: r for r in simp}
    common = [s for s in fmap if s in smap]
    N = len(common)

    fs = sum(is_solved(fmap[s]) for s in common)
    ss = sum(is_solved(smap[s]) for s in common)
    both = [s for s in common if is_solved(fmap[s]) and is_solved(smap[s])]
    uniq_full = sum(is_solved(fmap[s]) and not is_solved(smap[s]) for s in common)
    uniq_simp = sum(is_solved(smap[s]) and not is_solved(fmap[s]) for s in common)

    fexp_all = [num(fmap[s]["expansions"]) for s in common if num(fmap[s]["expansions"]) is not None]
    sexp_all = [num(smap[s]["expansions"]) for s in common if num(smap[s]["expansions"]) is not None]
    fexp_b = [num(fmap[s]["expansions"]) for s in both]
    sexp_b = [num(smap[s]["expansions"]) for s in both]
    fewer = sum(sx < fx for fx, sx in zip(fexp_b, sexp_b))
    more = sum(sx > fx for fx, sx in zip(fexp_b, sexp_b))
    W, p = wilcoxon(fexp_b, sexp_b, alternative="greater")

    fcov = st.mean(num(fmap[s]["bb_coverage"]) for s in common if num(fmap[s]["bb_coverage"]) is not None)
    scov = st.mean(num(smap[s]["bb_coverage"]) for s in common if num(smap[s]["bb_coverage"]) is not None)
    ftime = st.median(num(fmap[s]["sec"]) for s in common)
    stime = st.median(num(smap[s]["sec"]) for s in common)

    # McNemar for solve-rate difference (discordant pairs)
    from scipy.stats import binomtest
    mcp = binomtest(min(uniq_full, uniq_simp), uniq_full + uniq_simp, 0.5).pvalue
    print(f"=== EFFICIENCY (all {N} targets) ===")
    print(f"solved: full {fs}/{N} ({100*fs/N:.1f}%)  simplify {ss}/{N} ({100*ss/N:.1f}%)")
    print(f"solved by both: {len(both)}   unique full: {uniq_full}   unique simplify: {uniq_simp}"
          f"   (McNemar p={mcp:.3f})")
    print(f"mean expansions (all {N}): full {st.mean(fexp_all):.1f}  simplify {st.mean(sexp_all):.1f}"
          f"  ({100*(st.mean(fexp_all)-st.mean(sexp_all))/st.mean(fexp_all):.0f}% fewer)")
    print(f"mean expansions (solved-by-both n={len(both)}): full {st.mean(fexp_b):.1f}  "
          f"simplify {st.mean(sexp_b):.1f}  ({100*(st.mean(fexp_b)-st.mean(sexp_b))/st.mean(fexp_b):.0f}% fewer)")
    print(f"per-target: simplify fewer {fewer}, more {more}, tie {len(both)-fewer-more}")
    print(f"Wilcoxon (full>simplify, one-sided): W={W:.0f} p={p:.2e}")
    print(f"mean bb-coverage: full {fcov:.3f}  simplify {scov:.3f}")
    print(f"median time/target: full {ftime:.2f}s  simplify {stime:.2f}s")
...
def aizynth():
    ...
    full = {canon(r["smiles"]): is_solved(r) for r in load("full_1000.csv")}
    ...
    n_full = sum(full[k] for k in common if k in full)
    ...
    print(f"solved: AiZynth {n_az}/{n} ({100*n_az/n:.1f}%)  full {n_full}/{n} ({100*n_full/n:.1f}%)  "
...
def correlations():
    syn = {r["smiles"]: num(r["bb_coverage"]) for r in load("full_1000.csv")}
    solv = {r["smiles"]: is_solved(r) for r in load("full_1000.csv")}
    ...
    syn_sec = [num(r["sec"]) for r in load("full_1000.csv") if num(r["sec"]) is not None and r["status"] == "ok"]
